    ...
    <>
          ...
          {paragraphs.map((line, i) => {
            if (line.startsWith("## ")) {
              return (
                <h2
                  key={i}
                  className="font-mono text-2xl font-bold text-primary mt-10 mb-4"
                >
                  {line.replace("## ", "")}
                </h2>
              );
            ...
            if (line.startsWith("**") && line.endsWith("**")) {
              return (
                <p key={i} className="font-semibold text-primary mt-6 mb-1">
                  {line.replace(/\*\*/g, "")}
                </p>
              );
            }
            if (line.startsWith("- ")) {
              return (
                <li
                  key={i}
                  className="text-gray-600 leading-relaxed ml-4 list-disc"
                >
      ...
      {related.length > 0 && (
        <section className="px-4 md:px-page py-16 bg-gray-50">
          <h2 className="font-mono text-2xl font-bold text-primary mb-8">
            Related Articles
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {related.map((r) => (
              <Link
                key={r.id}
                href={`/blog/${r.id}`}
                className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow flex flex-col sm:flex-row"
              >
